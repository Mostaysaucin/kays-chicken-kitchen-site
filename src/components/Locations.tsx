...
import { LOCATIONS, PHONE_HREF } from "@/lib/constants";
    ...
    <section id="locations" className="section-padding" style={{ background: "var(--background-alt)" }}>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {locations.map((loc) => {
            const isSelected = hydrated && selectedLocation === loc.id;
            return (
              <div
                key={loc.id}
                ...
                style={{
                  background: "white",
                  boxShadow: isSelected ? "0 0 0 3px var(--primary), 0 2px 16px rgba(0,0,0,0.06)" : "0 2px 16px rgba(0,0,0,0.06)",
                }}
              >
                {/* Card Header */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {isSelected && (
                      <span
                        ...
                        style={{ fontFamily: "var(--font-heading)", textTransform: "uppercase", letterSpacing: "0.06em", background: "var(--secondary)", color: "var(--text-on-secondary)" }}
                      >
                        Your Location
                      </span>
                    )}
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <div className="flex items-center gap-2 mb-5">
                    <div className="flex gap-0.5" role="img" aria-label={`${loc.googleRating} out of 5 stars`}>
                      ...
                      <svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24">
                        <defs>
                          <linearGradient id={`star-${loc.id}`}>
                            <stop offset={loc.id === "causeway" ? "20%" : "60%"} stopColor="var(--secondary)" />
                            <stop offset={loc.id === "causeway" ? "20%" : "60%"} stopColor="#ccc" />
                          </linearGradient>
                        </defs>
                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" fill={`url(#star-${loc.id})`} />
                      </svg>
                    </div>
                  ...
                  <div className="mb-5">
                    ...
                    {loc.hoursNote && (
                      <p className="text-xs mt-2" style={{ color: "var(--text-secondary-on-light)", fontStyle: "italic" }}>
                        {loc.hoursNote}
                      </p>
                    )}
                  </div>
                  ...
                  {/* Payment Note */}
                  {loc.paymentNote && (
                    <div className="mb-5 py-2 px-3 rounded-md text-xs font-semibold" style={{ background: "rgba(221,40,3,0.05)", color: "var(--primary)", fontFamily: "var(--font-heading)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                      {loc.paymentNote}
                    </div>
                  )}
